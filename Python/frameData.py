...
def processData():
    ...
    prezSimple   = pd.read_csv(
            tbl('1976-2020-president.csv'),
            thousands=","
        ).filter(
            items = keepFromCSV[:2] + keepFromCSV[6:]
        )
    senateSimple = pd.read_csv(
            tbl('1976-2020-senate.csv'), 
            thousands=",", 
            encoding='ISO-8859-1'
        ).filter(
            items = keepFromCSV[:2] + [keepFromCSV[3]] + keepFromCSV[6:]
        )
    houseSimple = pd.read_csv(
            tbl('1976-2020-house.csv'), 
            thousands="," , 
            encoding='ISO-8859-1'
        ).filter(
            items = keepFromCSV
            ...
            for dist in districts:
                for yr in years:
                    ...
                    try:
                        ...
                        runUp  = df.iloc[
                                pd.to_numeric(
                                    elec[elec["candidatevotes"] != first]["candidatevotes"]
                                    ).idxmax()
                            ]
    ...
    calculated = {
            "prez":process(prezSimple), 
            "senate":process(senateSimple), 
            "house":process(houseSimple, 1)
        }
    for k in calculated.keys():
        calculated[k] = calculated[k].rename(
                columns = {
                        "candidate":"winner",
                        "candidatevotes":"winVotes", 
